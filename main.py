...
5. 推播 / 立即推播 / 手動推播 + 任務名稱（無附 URL）
   → {"action":"manual_push","task_name":"XXX"}

6. 推播 / 發送 + URL（訊息中含 http）
   → {"action":"push_url","url":"https://...","message":"前綴說明（選填）"}

7. 執行工具 / 啟動分析 / 跑分析 + 工具名稱（需要 Mac 本地執行）
   → {"action":"trigger_local","task_name":"XXX"}

可用排程任務（task_name for update_schedule/manual_push）：
