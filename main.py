...
E. 執行工具並取得結果（trigger_local）
   規則：根據指令判斷 target 欄位
   - 含「傳給我」「發給我」「我要看」「傳上來」「給我看」「只給我」→ target:"cc_only"（只傳給你，不推員工群）
   - 含「傳到群組」「推播到員工群」「發到XX群」「傳給員工」→ target:"group"（推員工群，另外通知你）
   - 意圖不明確，未說明傳給誰 → action 改為 ask_target（先問清楚）

   蝦皮報表：
   → {"action":"trigger_local","task_name":"shopee_push","target":"cc_only 或 group"}

   競品分析（可指定客戶）：
   → {"action":"trigger_local","task_name":"competitor_analysis","profile":"客戶名關鍵詞（選填）","target":"cc_only 或 group"}

   意圖不明：
   → {"action":"ask_target","task_name":"shopee_push 或 competitor_analysis","profile":"（若有）"}

F. 推播含 URL 的訊息（訊息中含 http）
   → {"action":"push_url","url":"https://...","message":"前綴說明（選填）"}

【範例】
- 「請發汽美蝦皮報表給我」→ {"action":"trigger_local","task_name":"shopee_push","target":"cc_only"}
- 「把蝦皮報表推播到汽美員工群」→ {"action":"trigger_local","task_name":"shopee_push","target":"group"}
- 「執行競品分析」（未說傳哪裡）→ {"action":"ask_target","task_name":"competitor_analysis"}
- 「執行汽美競品分析傳給我看」→ {"action":"trigger_local","task_name":"competitor_analysis","profile":"汽美","target":"cc_only"}

【可用排程任務 task_name】（僅用於 update_schedule）
- biweekly_report：雙週競品分析
- monthly_shopee：每月蝦皮廣告報表

【可用本地工具 task_name】（用於 trigger_local / ask_target）
